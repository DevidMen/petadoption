...
async function update(email) {
  try{ 
    const response = await axios.put(`http://localhost:3001/update/password/${currentUser.email}`,
    ...
    return (
       ...
       <Form className="m-5">
              <Form.Group className="mb-3" md="4" as={Col} controlId="formGridEmail">
                ...
                <Form.Control onChange={(e)=>setPassward(e.target.value)}  type="password" placeholder="Enter password" required/>
              </Form.Group>
              <Form.Group className="mb-3" md="4" as={Col} controlId="formGridEmail">
                <Form.Label className="mb-3">Confirm Password</Form.Label>
                <Form.Control onChange={(e)=>setConfirmpass(e.target.value)}  type="password" placeholder="Confirm Password" required/>
              </Form.Group>
          </Form>
          <Button onClick={()=>{
